...
;#SACMEX decision metrics max
  ...
  d_mantencion_max                     ;;distance from ideal point for decision to repare infrastructure
...
;#SACMEX decition metrics
  d_water_extraction
  d_mantencion                     ;;distance from ideal point for decision to repare infrastructure
...
;;actions sacmex
  ...
  water_distribution "09"

  ask agebs [
    set water_in_mc 0
    water_by_pipe  ;define if an ageb will receive water by pipe. Depends on tandeo,
    water_in_aday
    p_falla_infra
...
to update_globals  ;; update the maximum or minimum of values use by the model to calculate range of the value functions
  ...
  set d_mantencion_D_max max [d_mantencion] of agebs
...
to update_local [estado]  ;; update the maximum or minimum of values use by the model to calculate range of the value functions this time relative to the domain of the agent who needs the inforamtion to take decition
  ...
  set d_mantencion_D_max max [d_mantencion] of agebs with [CV_estado = estado]
    ...
    [
      if ?2 > 0.1 [
        ...
        if ? = "Escasez de agua"[
            ...
            set V replace-item i V (Value-Function (item i C1) map [ (1 / 20)  * ? ] scarcity_scale ["" "" "" ""] (item i C1_max) v_scale_S)
        ]
        ;###########################################################
        if ? = "Inundaciones"[
          set C1 replace-item i C1 [flooding] of myself
        ...
        if ? = "Eficacia del servicio" [
          if name_action = "Accion colectiva" [
            ...
            set V replace-item i V (Value-Function (item i C1) [0.9 0.94 0.97 0.99] ["" "" "" ""] (item i C1_max)  [0.0625 0.125 0.25 0.5 1])
          ]
          if name_action = "Modificacion vivienda"[
            set C1 replace-item i C1 [houses_with_dranage] of myself
            set C1_max replace-item i C1_max  infra_dranage_max ;change with update quantity for speed
 ...
 ;we set the value functions and define the distant metric based on compromisez programing function with exponent =2
  ...
  ask  agebs with [CV_estado = "09"][
    ...
    ask Alternatives_SACMEX [
      ...
      if name_action = "Mantenimiento"[
        ask myself[set d_mantencion ddd]
      ]
  ...
  ;#Actions of drenage
    ask Alternatives_SACMEX_D [
      ...
      if name_action = "Mantenimiento"[
        ask myself[set d_mantencion_D ddd]
      ]
    ]
  ]
...
;#############################################################################################################################################
  ...
  let Budget 0
  foreach sort-on [(1 - d_mantencion) ] agebs with [CV_estado = estado][    ;sort census blocks (+ (1 - densidad_pop / densidad_pop_max))
    ask ? [
...
to repair-Infra_D [estado]
    let Budget 0
    foreach sort-on [(1 - d_mantencion_D) ] agebs with [CV_estado = estado][ ;+ (1 - densidad_pop / densidad_pop_max)
      ask ? [
...
;#############################################################################################################################################
to water_distribution [estado] ;distribution of water from government by truck
let available_trucks Recursos_para_distribucion
...
;############################################################################################
      if visualization = "Infraestructura Edad Ab." and ticks > 1 [
        set shape "square"
        set color  scale-color turquoise Antiguedad-infra_Ab  (30 * 365) Antiguedad-infra_Ab_max
      ]
;############################################################################################
      if visualization = "Infraestructura Edad D" and ticks > 1 [
        set shape "square"
...
;           print item 6 (item 7 MMMCb_limit)
  ...
  set jj jj + 1
       ...
       ask Alternatives_MC with [name_action = "Compra de agua"] [set v_scale_S [0.05 0.1 1 0.05 0.05]]
       ask Alternatives_MC with [name_action = "Movilizaciones"] [set v_scale_S [0.05 0.05 0.05 0.4 1]]
       ask Alternatives_MC with [name_action = "Accion colectiva"] [set v_scale_S [0.05 0.1 1 1 1]]
...
;print matrix:pretty-print-text MMSACMEX_weighed_S
       ...
       set jj 0
       let MMSACMEX_weighted_D []
       let MMSACMEX_limit_D []
       let VSACMEX_limit_D []
       set cc 2
       set cri 2
       ...
       while [cri < 18][   ;tranfor the data from.csv to a matrix to manipulte using matrix extention
         ...
         set cri cri + 1
         set MMSACMEX_limit_D lput VSACMEX_limit_D MMSACMEX_limit_D
         set VSACMEX_limit_D []
       ]
       set MMSACMEX_weighted_D matrix:from-row-list MMSACMEX_limit_D

       let MMSACMEX_limit_D1 matrix:eigenvectors MMSACMEX_weighted_D

       let MMSACMEX_limit_D2 matrix:real-eigenvalues MMSACMEX_weighted_D
       set MMSACMEX_limit_D (matrix:times MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D MMSACMEX_weighted_D)
       ...
       foreach actions [
         create-Alternatives_SACMEX_D 1[    ;create an alternative, the criteria and the weights. Also in the case of HNP network the weight of each alternative in the limit matrix (alpha)
           ...
           let w_sum sum (list matrix:get MMSACMEX_limit_D 2 2
             matrix:get MMSACMEX_limit_D 3 2
             matrix:get MMSACMEX_limit_D 4 2
             matrix:get MMSACMEX_limit_D 5 2
             matrix:get MMSACMEX_limit_D 6 2
             matrix:get MMSACMEX_limit_D 7 2
             matrix:get MMSACMEX_limit_D 8 2
             matrix:get MMSACMEX_limit_D 9 2
             matrix:get MMSACMEX_limit_D 10 2
             matrix:get MMSACMEX_limit_D 11 2
             matrix:get MMSACMEX_limit_D 12 2
             matrix:get MMSACMEX_limit_D 13 2
             matrix:get MMSACMEX_limit_D 14 2
             matrix:get MMSACMEX_limit_D 15 2
             )
           set w_C1 (list (matrix:get MMSACMEX_limit_D 2 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 3 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 4 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 5 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 6 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 7 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 8 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 9 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 10 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 11 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 12 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 13 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 14 2 / w_sum)
             (matrix:get MMSACMEX_limit_D 15 2 / w_sum))
     ...
     ask Alternatives_SACMEX [set domain "09"] ;set key for municipality that sacmex has influence on
     ask Alternatives_SACMEX_D [set domain "09"] ;set key for municipality that sacmex has influence on
;#############################################################################################################################################
;#############################################################################################################################################
...
;##############################################################################################################
to supermatrix
 ;matrix:set MMSACMEX_weighted_D 9 0 0.814212784
end
...
<experiments>
  <experiment name="experiment1" repetitions="5" runMetricsEveryStep="false">
    ...
    <metric>mean [Antiguedad-infra_D] of agebs</metric>
    <metric>mean [scarcity_index] of agebs</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Gustavo A. Madero"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Venustiano Carranza"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Iztacalco"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Tlhuac"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Miguel Hidalgo"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Tlalpan"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Iztapalapa"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Coyoacn"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Benito Jurez"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Cuajimalpa de Morelos"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="lvaro Obregn"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Cuauhtmoc"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="Milpa Alta"]</metric>
    <metric>mean [Antiguedad-infra_Ab] of agebs with [name_delegation ="La Magdalena Contreras"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Gustavo A. Madero"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Venustiano Carranza"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Iztacalco"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Tlhuac"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Miguel Hidalgo"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Tlalpan"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Iztapalapa"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Coyoacn"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Benito Jurez"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Cuajimalpa de Morelos"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="lvaro Obregn"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="La Magdalena Contreras"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Cuauhtmoc"]</metric>
    <metric>mean [Antiguedad-infra_D] of agebs with [name_delegation ="Milpa Alta"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Gustavo A. Madero"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Venustiano Carranza"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Iztacalco"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Tlhuac"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Miguel Hidalgo"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Tlalpan"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Iztapalapa"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Coyoacn"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Benito Jurez"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Cuajimalpa de Morelos"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="lvaro Obregn"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Cuauhtmoc"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="Milpa Alta"]</metric>
    <metric>mean [scarcity_index] of agebs with [name_delegation ="La Magdalena Contreras"]</metric>
