...
load(file = paste(getwd(),"/MEGADAPT_APP/system_models/flooding_ponding/encharcamientos/mod_en_reg1.rda",sep=""))
for (i in 1:9){
load(file = paste(getwd(),sprintf("/MEGADAPT_APP/system_models/flooding_ponding/encharcamientos/mod_en_reg%s.rda",i),sep=""))
}
 

for (i in 1:9){
  load(file = paste(getwd(),sprintf("/MEGADAPT_APP/system_models/flooding_ponding/inundaciones/mod_inund_reg%s.rda",i),sep=""))
} 
