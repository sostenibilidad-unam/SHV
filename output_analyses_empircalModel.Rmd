...
promedios_delega<-aggregate( . ~ Delegation, data=g1_d, FUN=mean)

 O1<-ggplot(promedios_delega,aes(x=Delegation,y=Scarcity))+geom_boxplot()+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_x_discrete(labels=names[ order(promedios_delega$Scarcity)],limits=order(promedios_delega$Scarcity))+
