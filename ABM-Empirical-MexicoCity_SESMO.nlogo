...
to repair-Infra_D [estado]
  ...
  if item 0 [alternative_weights] of alternatives_WaterOperator_D with [name_action = "Nueva_infraestructura"] > item 0 [alternative_weights] of alternatives_WaterOperator_D with [name_action = "Mantenimiento"][ ;when favors new infra
    ask max-n-of Budget_N (agebs with [d_mantenimiento_D < d_new_D and CV_estado = estado and investment_here_D_new = 0 and investment_here_D_mant = 0]) [d_new_D][
      ...
      set Capacidad_d Capacidad_d + Phi_N
      set houses_with_dranage houses_with_dranage + 0.1 * (1 - houses_with_dranage)
    ]
    ask max-n-of Budget_M (agebs with [investment_here_D_new =  0 and investment_here_D_mant = 0 and CV_estado = estado]) [d_mantenimiento_D][
      let old_Age-infra_D Age-infra_D
      set investment_here_D_mant 1
      set investment_here_D 1
      set investment_here_accumulated_D investment_here_accumulated_D + 1
      set investment_here_accumulated_D_mant investment_here_accumulated_D_mant + 1
      set Age-infra_D Age-infra_D - Age-infra_D * Phi_M
      set garbage garbage - garbage * phi_G
    ]
  ]
  ...
  if item 0 [alternative_weights] of alternatives_WaterOperator_D with [name_action = "Mantenimiento"] > item 0 [alternative_weights] of alternatives_WaterOperator_D with [name_action = "Nueva_infraestructura"][ ;when favors mantenance
    ...
    ask max-n-of Budget_M (agebs with [d_mantenimiento_D > d_new_D and investment_here_D_new =  0 and investment_here_D_mant = 0 and CV_estado = estado]) [d_mantenimiento_D][
      let old_Age-infra_D Age-infra_D
      set investment_here_D_mant 1
      set investment_here_D 1
      set investment_here_accumulated_D investment_here_accumulated_D + 1
    ...
    ask max-n-of Budget_N (agebs with [CV_estado = estado and investment_here_D_new = 0 and investment_here_D_mant = 0]) [d_new_D][
      let old_Capacidad_D Capacidad_D
      set investment_here_D 1
      set investment_here_accumulated_D investment_here_accumulated_D + 1
      set investment_here_D_new 1
...
to change_subsidence_rate
    set subsidence subsidence + 0.1
end
;/subsidence;
;##############################################################################################################
to condition_infra_change
  set Age-infra_D Age-infra_D + 1
  set Capacidad_D Capacidad_D - decay_capacity
  if Capacidad_D < 0[set Capacidad_D 0]
end
;##############################################################################################################
to export-map
  ;This procedure creates a text files with a vector containing a particular atributes from the census blocks

  ifelse switch_MCDA = TRUE [
    set fn (word ticks "-" (word "DC" "-" (word Budget_N "-" (word Budget_M "-" (word Phi_M "-" (word Phi_N ".txt"))))))
  ]
  [
    set fn (word ticks "-" (word "SC" "-" (word Budget_N "-" (word Budget_M "-" (word Phi_M "-" (word Phi_N ".txt"))))))
  ]
    ...
    ask ?
      [
        ...
        file-write Age-infra_D
        file-write ponding                            ;;number of floods
...
to export-map_dvalues
 ...
 ifelse MCDA = "Favors New Infrastructure" [
    set fn (word ticks (word "FN-dvalue" "-" (word Budget_N "-" (word Budget_M "-" (word Phi_M "-" (word Phi_N ".txt"))))))
  ]
  [
    set fn (word ticks (word "FM-dvalue" "-" (word Budget_N "-" (word Budget_M "-" (word Phi_M "-" (word Phi_N ".txt"))))))
...
to Landscape_visualization ;;TO REPRESENT DIFFERENT INFORMATION IN THE LANDSCAPE
  set Age-infra_D_min min [Age-infra_D] of agebs with [CV_estado = "09"]
  set Age-infra_D_max max [Age-infra_D] of agebs with [CV_estado = "09"]
  ...
  ask agebs with [CV_estado = "09"] [
    ...
    ;############################################################################################
    if visualization = "Age Infraestructura D" and ticks > 1 [
      set size 2
      set color scale-color sky Age-infra_D Age-infra_D_min Age-infra_D_max
    ]
    ;############################################################################################
    if visualization = "Subsidence" and ticks > 1 [
      set size 2
      set color  scale-color magenta subsidence 0 subsidence_max
    ]
    ;############################################################################################
...
to flooding_InfPoiss
  ...
  set NE map [i -> item 0 [Age-infra_D] of agebs with [ID = i]] IDD
  r:put "new_Edad" NE
...
Visualization
Visualization
"d_M" "d_N" "GoogleEarth" "Ponding" "% houses with drainage" "P. Falla D" "Capacidad_D" "Age Infraestructura D" "Subsidance" "value_function_Age_d" "value_function_ponding" "value_function_falta_d" "value_function_capasity" "value_function_precipitation" "Action_New" "Action_Mant"
3
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [Age-infra_D] of agebs with [CV_estado = \"09\"]"
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" "histogram [Age-infra_d] of agebs with [CV_estado = \"09\"]"
